...
`/schedine` apre la scelta della famiglia e carica le **schedine multi-leg** del giorno (escluse le scalate): Generiche (profili classici), Solo PLAY (solo tip con valore alto / etichetta PLAY), Mercati forti (solo vincitore 1° set) o Selettive (poche scelte prudenti). Oltre ai pulsanti accetta `/schedine generiche|play|forti|selettive`. L’intro spiega sempre la famiglia selezionata e include data, legende stati/valore, ultimo aggiornamento e disclaimer. Pick void escludono la quota dalla combinata effettiva.

`/scalate` espone le stesse quattro famiglie, tramite pulsanti o `/scalate generiche|play|forti|selettive`, e carica le **scalate progressive** dello stesso giorno (stesso entitlement/feature flag delle schedine): step singoli in ordine di orario con reinvestimento del ritorno; PNG con colonna puntata step. Se non ci sono abbastanza candidati per la famiglia scelta, risponde con uno stato vuoto dedicato che ne ripete il significato.
